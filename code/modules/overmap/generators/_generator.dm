...
/datum/overmap_generator/New()
	overmap_size = global.using_map.overmap_size
	all_map_tiles = list()
	event_whitelist = list()
	empty_map_tiles = list()
...
/datum/overmap_generator/proc/build_overmap()
	...
	var/list/edges = list()
	for (var/square in block(locate(1,1,global.using_map.overmap_z), locate(overmap_size, overmap_size, global.using_map.overmap_z)))
		var/turf/T = square
	...
	spawn_events(global.using_map.overmap_z)
...
/datum/overmap_generator/proc/place_overmap_item_at(obj/O, x, y)
	var/turf/T = locate(x, y, global.using_map.overmap_z)
