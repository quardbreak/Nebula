...
do { \
	...
	proc2run ;\
	log_debug("\[MAZE] Operation '[opname]' on maze at [##x],[##y],[##z] took [round(0.1 * (REALTIMEOFDAY - timer), 0.1)]s"); \
} while (FALSE)
...
/obj/effect/mazegen/generator/Initialize(mapload)
	. = ..()
	SSmisc_late.maze_generators += src
...
/obj/effect/mazegen/generator/proc/run_generator()
	...
	log_debug("\[MAZE] Generation of maze at [x],[y],[z] complete within [round(0.1 * (REALTIMEOFDAY - total_time), 0.1)]s")
	qdel(src)
...
/obj/effect/mazegen/generator/proc/generate_path()
	...
	for(var/target_x in 0 to (width - 1)) // -1 so it spawns on the right tile
		for(var/target_y in 0 to (height - 1))
			...
			// Windows time
			var/obj/structure/window/reinforced/crescent/maze/WN = new(T)
			WN.dir = NORTH
			var/obj/structure/window/reinforced/crescent/maze/WS = new(T)
			WS.dir = SOUTH
			var/obj/structure/window/reinforced/crescent/maze/WE = new(T)
			WE.dir = EAST
			var/obj/structure/window/reinforced/crescent/maze/WW = new(T)
			WW.dir = WEST
			CHECK_TICK
		CHECK_TICK
	...
	while(length(working_stack))
		...
		if(length(unvisited_neighbours))
			...
			for(var/obj/structure/window/reinforced/crescent/maze/W in T)
				if(W.dir == text2num(D))
					qdel(W)

			// On both tiles
			for(var/obj/structure/window/reinforced/crescent/maze/W in T3)
				if(W.dir == global.reverse_dir[text2num(D)])
					qdel(W)
